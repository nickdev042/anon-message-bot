...
from app.states import Send_message, Answer_message, Find_out_who
...
async def handle_start_with_link(message: Message, command: CommandObject, state: FSMContext):
    await state.clear()
    ...
    if not tg_id_2:
        await message.answer("Данного пользователя не существует или ссылка недействительна :C")
        return
...
async def cmd_messaging(message: Message, state: FSMContext):
    ...
    await bot.copy_message(caption = "Отправленное фото: ",
                           chat_id = tg_id_2,
...
async def handle_reply_message(callback: CallbackQuery, state: FSMContext):
    ...
    if DB.check_user_VIP(callback.from_user.id):
        await callback.message.answer('Это сообщение прислал '
                                       f'<a href="tg://user?id={user}">{nick}</a>\n\n'
                                      )
    else:
...
async def cmd_messaging(message: Message, state: FSMContext):
    ...
    await bot.send_message(text = "Пользователь ответил тебе на сообщение!: ", chat_id = receiver_tg_id)
    await bot.copy_message(caption = "Отправленное фото: ",
